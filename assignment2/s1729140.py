...
#modules from computing Conditional Probability Distribution and Conditional Frequency Distribution
...
class HMM:
    ...
    def emission_model(self, train_data):
        ...
        for sent in train_data:   #for each sentence 
            for tuples in sent:    #for each pair of (word,tag) in every sentence
                data.append((tuples[1], tuples[0].lower()))     #list of tuples(tag,word)


        emission_FD = ConditionalFreqDist(data)
        # this is the estiamtor used for probability distribution
        est = lambda emission_FD:LidstoneProbDist(emission_FD,0.01,emission_FD.B()+1)
    ...
    def transition_model(self, train_data):
        ...
        est = lambda transition_FD :LidstoneProbDist(transition_FD,0.01,transition_FD.B()+1)
    ...
    def initialise(self, observation):
        ...
        #the viterbi data structure is a dictionary which takes states as key
        #and a list of costs as value
        #where cost is the negative sum of the prob of a word given a state
        #and the prob of transitioning from the previous state to the next state
        #viterbi[s] - list of probabilities for state s
        #viterbi[s][w] - the probability of word w in state s
        self.viterbi = {}
        
        for s in self.states:
            cost_of_transition = self.tlprob("<s>",s)
            cost_of_emission = self.elprob(s,observation)
            self.viterbi.update({s:[-(cost_of_emission+cost_of_transition)]})
            
        #print(self.viterbi["NOUN"][0])
            
        #the backpointer data strictire is a dictionary which takes states as key
        #and a list of states as value
        #is responsabile for tracking the path 
        #for every word and every state stores the path of states until that point
        self.backpointer = {}
        # Initialise step 0 of backpointer
        for state in self.states:
            self.backpointer.update({state:[0]})
              #NOUN
    ...
    def tag(self, observations):
        ...
        for i in range(1,len(observations)): # for each word in the sentence(omiting the first word)
            ...
            for s in self.states: # for each state
                list_of_total_costs = []
                for s_prime in self.states:  #loop again through all the states 
                    emission_cost = self.elprob(s,t)
                    transition_cost = self.tlprob(s_prime,s)  # transition from the previous tag to the recent tag
                    total_cost = self.get_viterbi_value(s_prime,t_index) - (emission_cost + transition_cost)
                    #print(total_cost)
                    list_of_total_costs.append(total_cost)
                self.viterbi[s].append(min(list_of_total_costs))   #after looping through all the states
                # add the lowest cost to the list
        ...
        for words in range(len(observations)-1):
            
            tags.append(self.backpointer[bestpath][len(observations)-1-words])
            bestpath = self.backpointer[bestpath][len(observations)-1-words]
        ...
        return tags[::-1]   #return the best path
...
def answer_question4b():
    ...
    tagged_sequence = [('In', 'ADP'), ('every', 'DET'), ('aspect', 'NOUN'), ('of', 'ADP'), ('service', 'NOUN'), ('--', '.'), ('to', 'ADP'), ('the', 'DET'), ('public', 'ADJ'), (',', '.'), ('to', 'ADP'), ('children', 'NOUN'), ('in', 'ADP'), ('schools', 'NOUN'), (',', '.'), ('to', 'ADP'), ('colleges', 'NOUN'), ('and', 'CONJ'), ('universities', 'NOUN'), ('--', '.'), ('the', 'DET'), ('library', 'NOUN'), ('of', 'ADP'), ('today', 'NOUN'), ('is', 'VERB'), ('failing', 'VERB'), ('to', 'ADP'), ('render', 'X'), ('vitally', 'X'), ('needed', 'X'), ('services', 'X'), ('.', 'X')]
    correct_sequence = [('In', 'ADP'), ('every', 'DET'), ('aspect', 'NOUN'), ('of', 'ADP'), ('service', 'NOUN'), ('--', '.'), ('to', 'ADP'), ('the', 'DET'), ('public', 'NOUN'), (',', '.'), ('to', 'ADP'), ('children', 'NOUN'), ('in', 'ADP'), ('schools', 'NOUN'), (',', '.'), ('to', 'ADP'), ('colleges', 'NOUN'), ('and', 'CONJ'), ('universities', 'NOUN'), ('--', '.'), ('the', 'DET'), ('library', 'NOUN'), ('of', 'ADP'), ('today', 'NOUN'), ('is', 'VERB'), ('failing', 'VERB'), ('to', 'PRT'), ('render', 'VERB'), ('vitally', 'ADV'), ('needed', 'VERB'), ('services', 'NOUN'), ('.', '.')]
    # Why do you think the tagger tagged this example incorrectly?
    answer =  inspect.cleandoc("""\
    We could observe that in this case a few words were wrongly tagged by our model as 'X'(foreign word). This might be the case because of two cases. Either the transition probability from X to X is very high in our corpus or the emission probability from X to word is very high.""")[0:280]
...
def answer_question5():
    ...
    There are several ways of handling the issue presented above. One of them would be to have an average
    ...
    distribution which would give enough weight to those missing values. """)[0:500]
...
def answer_question6():
    ...
    return inspect.cleandoc("""\
    We know that the clompexity of our model is TxS^2 where T is the number of words and S is the number
    ...
    a higher accuracy in the end.""")[0:500]
...
def answers():
    ...
    for sentence in test_data_universal:
        ...
        counter +=1
        if(counter <=10):
            mytags = [(word,tag) for ((word,_),tag) in zip(sentence,tags)]
            #print(mytags)
            #print(sentence)
            #print('\n')


    accuracy = correct/(correct+incorrect) # the accuracy is off by 0.02
    #probably I am only missing a sentence which makes it very hard to debug and to fix
