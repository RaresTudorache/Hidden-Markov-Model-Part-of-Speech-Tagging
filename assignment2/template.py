...
class HMM:
    ...
    def initialise(self, observation):
        ...
        #the viterbi data structure is a matrix(viterbi[s][w]) 
        #that stores the cost
        ...
        #viterbi[s] - list of states
        #viterbi[s][w] - total cost
        self.viterbi = [{}]
        self.backpointer = {}
        ...
        for s in self.states:
            ...
            self.viterbi[0][s] = -(cost_of_emission+cost_of_transition)
            self.backpointer[s] = 0
            
        #print(viterbi)
            
        #the backpointer ds is a matrix(backpointer[s][w])
        #that for every word and every state stores the path of states until that point
    ...
    def tag(self, observations):
        ...
        for t in range(1, len(observations)):
            ...
            for state in self.states:
                ...
                (probability, possible_state) = min(
                    [(-(self.viterbi[t - 1][prev_state] + self.tlprob(prev_state,state)
                      + self.elprob(state, observations[t])), prev_state) for prev_state in self.states])
        ...
        (_, state) = min([(self.viterbi[len(observations) - 1][state], state) for state in self.states])
